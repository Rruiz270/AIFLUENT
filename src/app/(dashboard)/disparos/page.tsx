...
interface LogEntry {
  ...
  campaign: string;
  message: string;
  status: "enviado" | "entregue" | "falha" | "lido" | "respondido";
  error?: string;
}
...
const channelIcon: Record<DisparoChannel, LucideIcon> = {
  whatsapp: MessageSquare,
  email: Mail,
  sms: Smartphone,
};
const channelLabel: Record<DisparoChannel, string> = {
  whatsapp: "WhatsApp",
  email: "E-mail",
  sms: "SMS",
};
...
function ChannelIcon({
  ...
  const colors: Record<DisparoChannel, string> = {
...
// Campanhas carregam de /api/campaigns (ver loadCampaigns). As abas abaixo
// (templates, audiencias, automacoes, logs, consent) ainda nao possuem backend.
...
const BACKEND_STATUS_MAP: Record<string, DisparoStatus> = {
  draft: "rascunho",
  scheduled: "agendada",
  running: "em_execucao",
  sending: "em_execucao",
...
function mapStatus(s: string): DisparoStatus {
  if (UI_STATUSES.includes(s)) return s as DisparoStatus;
  return BACKEND_STATUS_MAP[s] || "rascunho";
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapCampaign(c: any): Campanha {
  return {
    id: c.id,
    name: c.name,
    channel: (c.channel as DisparoChannel) || "whatsapp",
    status: mapStatus(c.status || "rascunho"),
    type: c.type === "sequencia" ? "sequencia" : "broadcast",
    ...
    audienceCount: c.totalRecipients || 0,
    delivered: c.totalDelivered || 0,
    read: c.totalOpened || 0,
    replied: c.totalReplied || 0,
...
export default function DisparosPage() {
  ...
  const patchCampaign = useCallback(
    async (id: string, status: DisparoStatus) => {
      try {
  ...
  const [automacoes, setAutomacoes] =
    useState<AutomacaoRule[]>(initialAutomacoes);
  ...
  const [wizardData, setWizardData] = useState({
    name: "",
    channel: "whatsapp" as DisparoChannel,
    type: "broadcast" as DisparoType,
    ...
    variables: [] as string[],
    attachments: false,
    whatsappButtons: false,
    whatsappListMsg: false,
    speed: 50,
    randomDelay: false,
    randomDelayMin: 5,
    randomDelayMax: 15,
    ...
    retryAuto: true,
    antiBlock: true,
    antiBlockRandomize: true,
    antiBlockVariations: true,
    antiBlockDelays: true,
    antiBlockDistribute: false,
    antiBlockPauseOnError: true,
    sendNow: true,
    scheduledDate: "",
    scheduledTime: "",
  });
  ...
  const [importedLeads, setImportedLeads] = useState<{
    fileName: string;
    count: number;
    status: "idle" | "pasting" | "success";
  }>({ fileName: "", count: 0, status: "idle" });
  ...
  const totalStats = useMemo(() => {
    const done = campaigns.filter(
      (c) => c.status === "concluida" || c.status === "em_execucao",
    );
    return {
    ...
    try {
      await fetch("/api/campaigns", {
        ...
        body: JSON.stringify({
          ...
          scheduledAt:
            !wizardData.sendNow && wizardData.scheduledDate
              ? `${wizardData.scheduledDate}T${wizardData.scheduledTime || "00:00"}`
              : null,
        }),
      });
    } catch {
      /* erro de rede — recarrega abaixo */
    }
    setWizardOpen(false);
    loadCampaigns();
  }
  ...
  async function toggleCampaignStatus(id: string) {
    ...
    await patchCampaign(id, next);
  }

  async function cancelCampaign(id: string) {
    await patchCampaign(id, "cancelada");
  }
  ...
  const eligibleCount = useMemo(() => {
    const selected = audiencias.filter((a) =>
      wizardData.audienceIds.includes(a.id),
    );
    const excluded = audiencias.filter((a) =>
      wizardData.excludeIds.includes(a.id),
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Gerencie campanhas de WhatsApp, E-mail e SMS
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 ml-auto">
              {(
                ...
                <button
                  key={s}
                  className="rounded-lg px-2.5 py-1 text-xs font-medium text-gray-500 hover:bg-gray-100 transition-colors"
                >
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          ...
                          {c.scheduledAt && (
                            <span>Agendada: {c.scheduledAt}</span>
                          )}
                          <span className="flex items-center gap-1">
                            <Users className="h-3 w-3" />{" "}
                            {c.audienceCount.toLocaleString("pt-BR")} contatos
                          </span>
                        </div>
                        {c.total > 0 && (
                          <div className="mt-3 max-w-md">
                            <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                              <span>
                                ...
                                {c.total.toLocaleString("pt-BR")} enviadas
                              </span>
                              <span>
                                {c.delivered.toLocaleString("pt-BR")} entregues
                              </span>
                            </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {(c.status === "em_execucao" ||
                        ...
                        <button
                          ...
                          className="rounded-lg p-2 text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors"
                          title={
                            c.status === "em_execucao" ? "Pausar" : "Retomar"
                          }
                        >
                          {c.status === "em_execucao" ? (
                      ...
                      {c.status !== "concluida" && c.status !== "cancelada" && (
                        <button
                          ...
                          className="rounded-lg p-2 text-gray-400 hover:bg-rose-50 hover:text-rose-500 transition-colors"
                          title="Cancelar"
                        >
                      ...
                      {(c.status === "concluida" ||
                        ...
                        <button
                          ...
                          className="rounded-lg p-2 text-gray-400 hover:bg-indigo-50 hover:text-indigo-500 transition-colors"
                          title="Ver relatorio"
                        >
              ...
              >
                ...
                <div className="flex items-center gap-1 flex-wrap mb-3">
                  {t.variables.map((v) => (
                    <span
                      key={v}
                      className="rounded bg-indigo-50 px-1.5 py-0.5 text-[10px] font-mono text-indigo-600"
                    >{`{{${v}}}`}</span>
                  ))}
                </div>
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="rounded p-1.5 text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors"
                    >
                ...
                >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 outline-none"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          defaultValue={editingTemplate.channel}
                        >
                    ...
                    <div>
                      ...
                      <textarea
                        rows={5}
                        className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none resize-none"
                    ...
                    <div className="flex items-center gap-1 flex-wrap">
                      ...
                      {availableVariables.map((v) => (
                        <button
                          key={v}
                          className="rounded bg-gray-100 px-2 py-0.5 text-[10px] font-mono text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
                        >{`{{${v}}}`}</button>
                      ))}
                    </div>
                  </div>
            ...
            {audiencias.map((a, i) => (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-1">
                    <button className="rounded p-1.5 text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors">
                      <Edit3 className="h-3.5 w-3.5" />
                    </button>
                    <button className="rounded p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-500 transition-colors">
                      <Trash2 className="h-3.5 w-3.5" />
        ...
        <TabsContent value="relatorios" className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <StatCard
              ...
              title="Enviadas"
              value={totalStats.enviadas}
              change={12.5}
              sparkline={[20, 35, 45, 42, 60, 72, 85]}
            />
            <StatCard
              icon={<CheckCircle2 className="h-4 w-4" />}
              title="Entregues"
              value={totalStats.entregues}
              change={10.2}
              sparkline={[18, 30, 40, 38, 55, 68, 80]}
            />
            <StatCard
              icon={<Eye className="h-4 w-4" />}
              title="Lidas"
              value={totalStats.lidas}
              change={8.7}
              sparkline={[15, 25, 35, 32, 45, 52, 60]}
            />
            <StatCard
              icon={<MessageSquare className="h-4 w-4" />}
              title="Respondidas"
              value={totalStats.respondidas}
              change={15.3}
              sparkline={[5, 12, 18, 15, 22, 30, 35]}
            />
            <StatCard
              icon={<XCircle className="h-4 w-4" />}
              title="Falhas"
              value={totalStats.falhas}
              change={-3.2}
              sparkline={[10, 8, 12, 6, 5, 7, 4]}
            />
          </div>
            ...
            {[
              {
                ...
                value:
                  totalStats.entregues > 0
                    ? (
                        (totalStats.entregues / totalStats.enviadas) *
                        100
                      ).toFixed(1)
                    : "0",
              ...
              },
              {
                ...
                value:
                  totalStats.entregues > 0
                    ? ((totalStats.lidas / totalStats.entregues) * 100).toFixed(
                        1,
                      )
                    : "0",
                color: "text-sky-600",
              ...
              },
              {
                ...
                value:
                  totalStats.lidas > 0
                    ? (
                        (totalStats.respondidas / totalStats.lidas) *
                        100
                      ).toFixed(1)
                    : "0",
              ...
              },
              {
                label: "Taxa de Falha",
                value:
                  totalStats.enviadas > 0
                    ? ((totalStats.falhas / totalStats.enviadas) * 100).toFixed(
                        1,
                      )
                    : "0",
                color: "text-rose-600",
              ...
              >
                ...
                <p className={cn("text-3xl font-bold tabular-nums", r.color)}>
                  {r.value}%
                </p>
              </motion.div>
            ))}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="space-y-4">
              {[
                ...
                {
                  channel: "email" as DisparoChannel,
                  sent: 13900,
                  delivered: 13220,
                  read: 6610,
                  replied: 962,
                  ...
                  {campaigns
                    ...
                    .map((c) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                          {c.read.toLocaleString("pt-BR")}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                          {c.replied.toLocaleString("pt-BR")}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums font-medium text-indigo-600">
                          {c.read > 0
                            ? ((c.replied / c.read) * 100).toFixed(1)
                            : "0"}
                          %
                        </td>
                      </tr>
        ...
        <TabsContent value="automacoes" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-gray-900">
                Regras de Automacao Pos-Disparo
              </h3>
              <p className="text-xs text-gray-500 mt-0.5">
                Configure acoes automaticas baseadas no comportamento do
                destinatario
              </p>
            </div>
            <Button variant="outline">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    <div
                      className={cn(
                        "flex h-10 w-10 shrink-0 items-center justify-center rounded-lg",
                        a.active ? "bg-indigo-50" : "bg-gray-50",
                      )}
                    >
                      <Zap
                        className={cn(
                          "h-5 w-5",
                          a.active ? "text-indigo-500" : "text-gray-400",
                        )}
                      />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={cn(
                        ...
                        a.active ? "bg-indigo-500" : "bg-gray-200",
                      )}
                    >
                      <span
                        className={cn(
        ...
        <TabsContent value="lgpd" className="mt-6 space-y-6">
            ...
            {[
              {
                label: "Opt-in Ativos",
                value:
                  initialConsent.filter((c) => c.type === "opt_in").length *
                  1540,
                icon: <CheckCircle2 className="h-4 w-4" />,
                color: "text-emerald-600",
              },
              {
                label: "Opt-out",
                value:
                  initialConsent.filter((c) => c.type === "opt_out").length *
                  380,
                icon: <XCircle className="h-4 w-4" />,
              ...
              },
              {
                label: "Bloqueados",
                value:
                  initialConsent.filter((c) => c.type === "bloqueado").length *
                  95,
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
            ...
            <div className="space-y-3">
              {[
                {
                  label: "Descadastro automatico",
                  desc: "Remover automaticamente contatos que solicitam opt-out",
                  active: true,
                },
                {
                  label: "Verificacao de consentimento pre-envio",
                  desc: "Validar consentimento antes de cada disparo",
                  active: true,
                },
                {
                  label: "Registrar base legal",
                  desc: "Exigir base legal para cada campanha",
                  active: false,
                },
              ].map((cfg, i) => (
                <div
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      cfg.active ? "bg-indigo-500" : "bg-gray-200",
                    )}
                  >
                    <span
                      className={cn(
                        "inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform",
                        cfg.active ? "translate-x-6" : "translate-x-1",
                      )}
                    />
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              >
                ...
                <p className={cn("text-2xl font-bold tabular-nums", h.color)}>
                  {h.value}
                </p>
              </motion.div>
            ))}
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  23 mensagens aguardando reenvio
                </p>
              </div>
              <Button variant="outline" size="sm">
                <RefreshCw className="h-3.5 w-3.5" /> Reprocessar Tudo
              </Button>
            </div>
            <div className="flex items-center gap-3">
              {[
                    ...
                    <React.Fragment key={step}>
                      ...
                      {i < WIZARD_STEPS.length - 1 && (
                        <div
                          className={cn(
                            "h-px flex-1",
                            i < wizardStep ? "bg-emerald-300" : "bg-gray-200",
                          )}
                        />
                      )}
                    </React.Fragment>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 outline-none"
                          placeholder="Ex: Vestibular 2026 - Lembrete"
                        />
                      </div>
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {(
                            ["whatsapp", "email", "sms"] as DisparoChannel[]
                          ).map((ch) => (
                            <button
                              key={ch}
                              onClick={() =>
                                setWizardData((d) => ({ ...d, channel: ch }))
                              }
                              className={cn(
                                "flex flex-col items-center gap-2 rounded-xl border-2 p-4 transition-all",
                                wizardData.channel === ch
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            {
                              value: "broadcast" as DisparoType,
                              label: "Broadcast",
                              desc: "Envio unico para todos",
                            },
                            {
                              value: "sequencia" as DisparoType,
                              label: "Sequencia",
                              desc: "Mensagens em serie",
                            },
                          ].map((t) => (
                            <button
                              key={t.value}
                              onClick={() =>
                                setWizardData((d) => ({ ...d, type: t.value }))
                              }
                              className={cn(
                      ...
                      <div className="rounded-xl border-2 border-dashed border-sky-300 bg-sky-50 p-5">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              CSV, XLSX ou TXT com numeros/emails
                            </p>
                          </div>
                        </div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (file) {
                                  ...
                                  setWizardData((d) => ({
                                    ...d,
                                    audienceIds: [...d.audienceIds, "imported"],
                                  }));
                                  setImportedLeads({
                                    fileName: file.name,
                                    count,
                        ...
                        {importedLeads.status === "pasting" && (
                          <div className="mt-3 space-y-2">
                            ...
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-500">
                                {importedLeads.count} contatos detectados
                              </span>
                              <button
                                onClick={() =>
                                  setImportedLeads((prev) => ({
                                ...
                                className="px-3 py-1.5 bg-sky-500 hover:bg-sky-400 text-white text-xs rounded-lg transition-colors"
                              >
                                Confirmar
                              </button>
                            </div>
                          ...
                          importedLeads.count > 0 && (
                            <div className="mt-3 flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                              ...
                              <span className="text-sm text-emerald-700 font-medium">
                                {importedLeads.count.toLocaleString("pt-BR")}{" "}
                                contatos importados de {importedLeads.fileName}
                              </span>
                              <button
                                onClick={() =>
                                  setImportedLeads({
                        ...
                        <div className="space-y-2">
                          {audiencias.map((a) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={wizardData.audienceIds.includes(a.id)}
                                onChange={(e) => {
                                  const ids = e.target.checked
                                    ? [...wizardData.audienceIds, a.id]
                                    : wizardData.audienceIds.filter(
                                        (id) => id !== a.id,
                                      );
                                  setWizardData((d) => ({
                                    ...d,
                        ...
                        <div className="space-y-2">
                          {audiencias.map((a) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={wizardData.excludeIds.includes(a.id)}
                                onChange={(e) => {
                                  const ids = e.target.checked
                                    ? [...wizardData.excludeIds, a.id]
                                    : wizardData.excludeIds.filter(
                                        (id) => id !== a.id,
                                      );
                                  setWizardData((d) => ({
                                    ...d,
                    ...
                    >
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={8}
                          className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 outline-none resize-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50"
                          placeholder={
                            wizardData.channel === "email"
                              ? "Corpo do e-mail..."
                              : "Digite sua mensagem..."
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              onClick={() =>
                                setWizardData((d) => ({
                                  ...d,
                                  whatsappButtons: !d.whatsappButtons,
                                }))
                              }
                              className={cn(
                                "flex items-center gap-2 rounded-lg border px-3 py-2 text-xs transition-all",
                                wizardData.whatsappButtons
                                  ? "border-indigo-500 bg-indigo-50 text-indigo-700"
                                  : "border-gray-200 text-gray-500 hover:border-gray-300",
                              )}
                            >
                              <MousePointerClick className="h-4 w-4" /> Botoes
                              Interativos
                            </button>
                            <button
                              onClick={() =>
                                setWizardData((d) => ({
                                  ...d,
                                  whatsappListMsg: !d.whatsappListMsg,
                                }))
                              }
                              className={cn(
                                "flex items-center gap-2 rounded-lg border px-3 py-2 text-xs transition-all",
                                wizardData.whatsappListMsg
                                  ? "border-indigo-500 bg-indigo-50 text-indigo-700"
                                  : "border-gray-200 text-gray-500 hover:border-gray-300",
                              )}
                            >
                  ...
                  {wizardStep === 3 && (
                    ...
                    >
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Velocidade de Disparo: {wizardData.speed} msgs/min
                        </label>
                        <input
                          type="range"
                          min={1}
                      ...
                      <div className="flex items-center justify-between rounded-lg border border-gray-200 p-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900">
                            Intervalo aleatorio entre mensagens
                          </p>
                          <p className="text-xs text-gray-500">
                            Adiciona atraso variavel para parecer mais natural
                          </p>
                        </div>
                        <button
                          onClick={() =>
                      ...
                      {wizardData.randomDelay && (
                        <div className="grid grid-cols-2 gap-3 pl-4">
                          <div>
                            ...
                            <input
                              type="number"
                              value={wizardData.randomDelayMin}
                              onChange={(e) =>
                                setWizardData((d) => ({
                                  ...d,
                                  randomDelayMin: Number(e.target.value),
                                }))
                              }
                              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none"
                            />
                          ...
                          <div>
                            ...
                            <input
                              type="number"
                              value={wizardData.randomDelayMax}
                              onChange={(e) =>
                                setWizardData((d) => ({
                                  ...d,
                                  randomDelayMax: Number(e.target.value),
                                }))
                              }
                              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none"
                            />
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            type="number"
                            value={wizardData.limitPerHour}
                            onChange={(e) =>
                              setWizardData((d) => ({
                                ...d,
                                limitPerHour: Number(e.target.value),
                              }))
                            }
                            className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none"
                          />
                      ...
                      <div className="flex items-center justify-between rounded-lg border border-gray-200 p-4">
                        ...
                        <button
                          onClick={() =>
                            setWizardData((d) => ({
                              ...d,
                              retryAuto: !d.retryAuto,
                            }))
                          }
                          className={cn(
                            "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
                            wizardData.retryAuto
                              ? "bg-indigo-500"
                              : "bg-gray-200",
                          )}
                        >
                      ...
                      <div className="rounded-xl border border-gray-200 p-4 space-y-3">
                        ...
                        {wizardData.antiBlock && (
                          <div className="space-y-2 pt-2 border-t border-gray-100">
                            {[
                              {
                                key: "antiBlockRandomize" as const,
                                label: "Randomizacao de ordem de envio",
                              },
                              {
                                key: "antiBlockVariations" as const,
                                label: "Variacoes automaticas de texto",
                              },
                              {
                                key: "antiBlockDelays" as const,
                                label: "Delays aleatorios entre msgs",
                              },
                              {
                                key: "antiBlockDistribute" as const,
                                label: "Distribuir entre remetentes",
                              },
                              {
                                key: "antiBlockPauseOnError" as const,
                                label: "Pausar ao detectar taxa de erro alta",
                              },
                            ].map((opt) => (
                              <label
                                key={opt.key}
                          ...
                          [
                            {
                              value: true,
                              label: "Enviar Agora",
                              desc: "Iniciar imediatamente apos a criacao",
                              icon: Send,
                            },
                            {
                              value: false,
                              label: "Agendar",
                              desc: "Escolher data e horario para envio",
                              icon: Clock,
                            },
                          ] as const
                        ).map((opt) => (
                        ...
                        <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-gray-900">
                              {wizardData.name || "(sem nome)"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Canal:</span>
                            <span className="font-medium text-gray-900 flex items-center gap-1">
                              <ChannelIcon channel={wizardData.channel} />
                              {channelLabel[wizardData.channel]}
                            </span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-indigo-600">
                              {eligibleCount.toLocaleString("pt-BR")}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Velocidade:</span>
                            <span className="font-medium text-gray-900">
                              {wizardData.speed} msgs/min
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Janela:</span>
                            <span className="font-medium text-gray-900">
                              {wizardData.windowStart} - {wizardData.windowEnd}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-gray-900">
                              {wizardData.sendNow
                                ? "Imediato"
                                : `${wizardData.scheduledDate} ${wizardData.scheduledTime}`}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Retry:</span>
                            <span className="font-medium text-gray-900">
                              {wizardData.retryAuto ? "Sim" : "Nao"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-gray-900">
                              {wizardData.antiBlock ? "Ativo" : "Inativo"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                      ...
                      <div className="rounded-lg bg-amber-50 border border-amber-100 p-4 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-amber-600 mt-0.5">
                            Apos iniciar, a campanha sera enviada para{" "}
                            {eligibleCount.toLocaleString("pt-BR")} contatos.
